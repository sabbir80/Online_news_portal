...
<?php }?>
 ...
 <div class="popup" id="popup" style="background: rgba(0,0,0,0.6);width: 100%;height: 100%;position: absolute;top:0;justify-content: center;align-items: center;display:none;">
      <form method="post" action="single_page.php">
...
if(isset($_POST['submit'])){
  ...
  $query=mysqli_query($conn,"insert into user(name,email)values('$name','$email')");
  if($query){
...
?>
  ...
  document.querySelector(".popup").style.display="block";
    ...
    $conn =mysqli_connect("localhost","root", "","news");
    $id=$_GET['page'];
    $query=mysqli_query($conn,"SELECT * FROM published where id='$id'");
    while($row=mysqli_fetch_array($query)){
    ?>
          <div class="blog-post">
...
<div>
  ...
  <div class="row" style="margin-top: -8%;" >
   <div class="col-md-8">
<div class="card my-4" style="width: 700px;">
            <h5 class="card-header">Leave a Comment:</h5>
            <div class="card-body">
              <form name="Comment" method="post" action="user_cmnt.php? page=<?php echo $id;?>">
...
<div>
 ...
 $conn =mysqli_connect("localhost","root", "","news");
  $id=$_GET['page'];
  $sql=mysqli_query($conn,"SELECT * from approved where post='$id'");
    while($row=mysqli_fetch_array($sql)){ 
  ?>
 <img class="d-flex mr-3 rounded-circle" src="image/user1.png" alt="" height="30"width="30">
...
<h6><?php echo $row['comment'];?></h6>
 <?php }?> 
        ...
        <aside class="col-md-4 blog-sidebar">
          <div class="p-3 mb-3 bg-light rounded"style="margin-top: 200px;">
